...
guard args.count >= 2 else {
    fputs("Usage: render_restart_daily_images.swift <assets-dir>\n", stderr)
    exit(1)
}

let outDir = URL(fileURLWithPath: args[1], isDirectory: true)
let bg1 = NSColor(calibratedRed: 0.969, green: 0.956, blue: 0.926, alpha: 1)
let bg2 = NSColor(calibratedRed: 0.949, green: 0.958, blue: 0.972, alpha: 1)
...
do {
    do {
        ...
        drawText("今天已经没法离开 AI", rect: NSRect(x: 140, y: 148, width: 920, height: 64), font: .systemFont(ofSize: 54, weight: .bold), color: dark, align: .center, lineHeight: 62)
        drawText("谈成长", rect: NSRect(x: 140, y: 232, width: 920, height: 88), font: .systemFont(ofSize: 86, weight: .heavy), color: dark, align: .center, lineHeight: 90)
        drawText("但大多数人还没真正意识到这一点", rect: NSRect(x: 120, y: 348, width: 960, height: 68), font: .systemFont(ofSize: 48, weight: .bold), color: dark, align: .center, lineHeight: 58)
    ...
    do {
        ...
        rounded(NSRect(x: 78, y: 68, width: 1044, height: 484), radius: 28, fill: .white, stroke: dark, line: 4)
        drawText("这次重新日更", rect: NSRect(x: 140, y: 148, width: 920, height: 64), font: .systemFont(ofSize: 54, weight: .bold), color: dark, align: .center, lineHeight: 62)
        drawText("重心会放在 AI", rect: NSRect(x: 120, y: 232, width: 960, height: 88), font: .systemFont(ofSize: 82, weight: .heavy), color: dark, align: .center, lineHeight: 88)
        drawText("但写的仍然是 AI 时代的成长与工作", rect: NSRect(x: 120, y: 348, width: 960, height: 68), font: .systemFont(ofSize: 46, weight: .bold), color: dark, align: .center, lineHeight: 56)
        watermark("@Bill的精神时光屋", rect: NSRect(x: 860, y: 494, width: 190, height: 24), color: muted)
        r.finish()
        try save(r.image, "2")
    }
